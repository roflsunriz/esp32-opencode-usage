...
0.1.1以降はschema 2/3から4へ自動移行し、Wi-Fi・認証・既存の消灯時間を保持します（schema 2の消灯時間は1分）。初期の向きは通常向きです。起動後にBOOTを短く押して離し、180度反転、反転後のタブ・時間選択、再起動後の向きの保持を確認します。1.5秒以上の長押しではタッチ位置・押圧感度の調整が始まります。両方の向きでタブと時間を操作し、画面をいったん消して描き直すちらつきが発生しないことも確認します。調整値はNVSの `opencode-touch` に保存され、通常のファームウェア更新では引き継がれます。
