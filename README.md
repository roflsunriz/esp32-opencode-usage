...
- Wi-FiのSSIDとパスワード
- OpenCodeコンソールのセッションCookie（`__Host-console_session=`を含む値）
- ワークスペースID
- 取得間隔
...
PC側のログインはWindows DPAPIで保護して保存します。ESP32へ送る設定にはセッションCookieとワークスペースが含まれます。保存完了後、操作画面の「ESP32の直接更新を確認」を確認できればPCを終了できます。
...
本体の **BOOT** ボタンを短く押して離すと、画面が180度回転します。もう一度クリックすると元の向きへ戻ります。向きは自動保存され、電源を切っても保持されます。反転後も表示された位置でタッチ操作でき、消灯中のBOOTクリックは点灯と反転を同時に行います。

タッチ位置がずれる、またはタッチペンの軽い押下が反応しない場合は、起動後に **BOOTを1.5秒以上押して離して**ください。画面に表示される3つの十字を普段の強さで押して離すと、基板の軸方向・タッチ位置・押圧感度を保存します。押下が取得できない、または測定値が不正な場合は元の値を保ちます。抵抗膜を押せない静電容量式ペンは感度調整では反応しません。

この描画・入力方式と校正はビルドとネイティブテストまで確認済みで、専用基板での操作確認は[検証手順](verification.md)に残っています。
...
画面は内容が変わった領域だけを更新し、操作時のちらつきを抑えます。
...
ESP32にはWi-Fiパスワード、セッションCookie、ワークスペースがNVSへ保存されます。通常のESP32フラッシュは暗号化されていないため、機器と全フラッシュバックアップを第三者へ渡さないでください。`.private/` はPC側の認証情報とバックアップを含むため、同期・共有・コミットの対象にしません。
