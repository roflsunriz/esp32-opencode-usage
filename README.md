...
LCD上部の **Display** タブで、消灯までの時間と取得間隔をいつでも変更できます。0〜59分のスライダー、0〜24時間のスライダー、60〜600秒（60秒刻み）の取得間隔スライダーをタップで調整し、その場で保存されます。タブ内はスクロールバーで上下に移動できます。**Usage** タブで使用量へ戻ります。

![Displayタブのスライダーと取得間隔](docs/lcd-display-sliders.png)

画面は内容が変わった領域だけを更新し、操作時のちらつきを抑えます。

0分0時間の指定は自動消灯オフ（常時点灯）と同じです。初期値は **1m** です。最後のタッチから設定時間が経過するとバックライトが消灯します。Wi-Fiの自動更新では時間を延長せず、消灯中も使用量の取得は続けます。Usageタブの状態表示には、更新状態と次の取得までの残り時間（例: `OpenCode updated - next 42s`）も表示されます。

消灯中の最初のタッチは再点灯だけに使います。タブや設定を操作する場合は、点灯後にもう一度タッチしてください。設定は再起動後も保持され、変更のためにPCを常時接続する必要はありません。

PCの初期設定画面の消灯時間は9択のままです。USB接続時は **消灯時間を保存** だけで既存のWi-Fi・認証を保持して変更でき、**画面を点灯** で手動点灯できます。本体のスライダーで9択以外の値を保存した場合、PC画面の選択欄は空表示になりますが、保存動作に影響はありません。
