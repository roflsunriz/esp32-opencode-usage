...
タッチ位置がずれる、またはタッチペンの軽い押下が反応しない場合は、起動後に **BOOTを1.5秒以上押して離して**ください。画面に表示される左上と右下の十字を普段使うペンで押して離すと、タッチ位置と押圧感度を保存します。押下が取得できない、または2点が不正な場合は元の値を保ちます。抵抗膜を押せない静電容量式ペンは感度調整では反応しません。
